  ...
  while(true) {
    ...
    if(PACKETSIZE) {
      ...
      LoRa.readBytes(PACKETBUFFER, PACKETSIZE);
        ...
        CACHEINDEX += 1;
        if(CACHEINDEX >= 25) { CACHEINDEX = 0; }
      } else if(PACKETBUFFER[0] == RREP_PACKET && PACKETBUFFER[1] < 25 && PACKETBUFFER[2] < MAX_NODES && PACKETBUFFER[3] == NODE_ID && PACKETBUFFER[4] == NODE_ID && PACKETBUFFER[5] != NODE_ID) {
        ...
        ROUTINGTABLE[PACKETBUFFER[4]] = {1, PACKETBUFFER[2], ROUTINGCACHE[PACKETBUFFER[1]].previousNode, PACKETBUFFER[4], PACKETBUFFER[5]};
      } else if(PACKETBUFFER[0] == RRER_PACKET && PACKETBUFFER[1] < 25 && PACKETBUFFER[2] < MAX_NODES && PACKETBUFFER[3] == NODE_ID && PACKETBUFFER[4] == NODE_ID && PACKETBUFFER[5] != NODE_ID) {
